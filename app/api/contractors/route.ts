...
export async function POST(request: Request) {
  ...
  if (!parsed.success) {
    return NextResponse.json(
      { message: "Invalid contractor payload.", issues: parsed.error.flatten() },
      { status: 400 }
    );
  }
  ...
  const contractor = await prisma.contractor.create({
    data: {
      ...
      companyIdentifier: emptyToUndefined(parsed.data.companyIdentifier)
        ? normalizeCompanyIdentifier(parsed.data.companyIdentifier!)
        : null,
